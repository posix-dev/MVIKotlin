...
Time travel is a power debugging tool. Basically it allows to record all events and states from all active `Stores`. When events are recorded you can explore them, replay and debug.
...
You can also combine it with the `LoggingStoreFactory` in the fllowing way:
...
Currently MVIKotlin provides only one way of using time travel. You can integrate special UI into your app which provides controlls for time travel and displays a list of recorded events. It is also possible to explore and debug any recorded event. Debuggins means you can put a breakpoint in your code and fire a previously recorded event. You can do usual debugging when the breakpoint is triggered.
